...
- 잘못된 동영상 경로와 예약 문자 파일명, 비어 있는 선택 소스, 안전하지 않은 외부 URL·포스터 경로, 음소거하지 않은 자동 재생과 재생 수단 없는 활성 동영상을 배포 전에 차단하는 회귀 테스트
- 동작 줄이기 환경의 초기 자동 재생을 방지하고 실행 중 설정 변경과 자동 재생 거부 시 컨트롤을 복구하는 접근성 처리
- 활성 동영상의 초기 HTML 속성, 포스터 이미지 형식과 외부 동영상 페이지 URL을 검증하는 회귀 테스트
